using System;
using System.Collections.Generic;
using System.IO;
using System.Threading;
using System.Threading.Tasks;

namespace Linear.Runtime;

/// <summary>
/// Structure layout
/// </summary>
public class Structure
{
    /// <summary>
    /// Name of this structure.
    /// </summary>
    public string? Name { get; }

    /// <summary>
    /// Default length of structure.
    /// </summary>
    public int? DefaultLength { get; }

    private readonly List<StructureMember> _members;

    /// <summary>
    /// Initializes an instance of <see cref="Structure"/>.
    /// </summary>
    /// <param name="name">Name of this structure.</param>
    /// <param name="defaultLength">Default length of structure.</param>
    /// <param name="members">Members of structure.</param>
    public Structure(string? name, int? defaultLength, List<StructureMember> members)
    {
        Name = name;
        DefaultLength = defaultLength;
        _members = members;
    }

    /// <summary>
    /// Parses structure from stream.
    /// </summary>
    /// <param name="registry">Structure registry.</param>
    /// <param name="stream">Stream to read from.</param>
    /// <param name="parseState">Current parse state.</param>
    /// <returns>Parsed structure.</returns>
    public StructureInstance Parse(IReadOnlyDictionary<string, Structure> registry, Stream stream, ParseState parseState)
    {
        StructureInstance instance = new(registry, parseState.Parent, parseState.Offset, parseState.Length ?? DefaultLength, parseState.Index);
        foreach (var member in _members)
        {
            var result = member.Initializer.Initialize(new StructureEvaluationContext(instance), stream);
            if (result.Discard) break;
        }
        return instance;
    }

    /// <summary>
    /// Parses structure from stream.
    /// </summary>
    /// <param name="registry">Structure registry.</param>
    /// <param name="stream">Stream to read from.</param>
    /// <param name="parseState">Current parse state.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>Parsed structure.</returns>
    public async ValueTask<StructureInstance> ParseAsync(IReadOnlyDictionary<string, Structure> registry, Stream stream, ParseState parseState, CancellationToken cancellationToken = default)
    {
        StructureInstance instance = new(registry, parseState.Parent, parseState.Offset, parseState.Length ?? DefaultLength, parseState.Index);
        foreach (var member in _members)
        {
            var result = await member.Initializer.InitializeAsync(new StructureEvaluationContext(instance), stream, cancellationToken);
            if (result.Discard) break;
        }
        return instance;
    }

    /// <summary>
    /// Parses structure from memory.
    /// </summary>
    /// <param name="registry">Structure registry.</param>
    /// <param name="memory">Buffer to read from.</param>
    /// <param name="parseState">Current parse state.</param>
    /// <returns>Parsed structure.</returns>
    public StructureInstance Parse(IReadOnlyDictionary<string, Structure> registry, ReadOnlyMemory<byte> memory, ParseState parseState)
    {
        StructureInstance instance = new(registry, parseState.Parent, parseState.Offset, parseState.Length ?? DefaultLength, parseState.Index);
        foreach (var member in _members)
        {
            var result = member.Initializer.Initialize(new StructureEvaluationContext(instance), memory);
            if (result.Discard) break;
        }
        return instance;
    }

    /// <summary>
    /// Parses structure from memory.
    /// </summary>
    /// <param name="registry">Structure registry.</param>
    /// <param name="memory">Buffer to read from.</param>
    /// <param name="parseState">Current parse state.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>Parsed structure.</returns>
    public async ValueTask<StructureInstance> ParseAsync(IReadOnlyDictionary<string, Structure> registry, ReadOnlyMemory<byte> memory, ParseState parseState, CancellationToken cancellationToken = default)
    {
        StructureInstance instance = new(registry, parseState.Parent, parseState.Offset, parseState.Length ?? DefaultLength, parseState.Index);
        foreach (var member in _members)
        {
            var result = await member.Initializer.InitializeAsync(new StructureEvaluationContext(instance), memory, cancellationToken);
            if (result.Discard) break;
        }
        return instance;
    }

    /// <summary>
    /// Parses structure from span.
    /// </summary>
    /// <param name="registry">Structure registry.</param>
    /// <param name="span">Buffer to read from.</param>
    /// <param name="parseState">Current parse state.</param>
    /// <returns>Parsed structure.</returns>
    public StructureInstance Parse(IReadOnlyDictionary<string, Structure> registry, ReadOnlySpan<byte> span, ParseState parseState)
    {
        StructureInstance instance = new(registry, parseState.Parent, parseState.Offset, parseState.Length ?? DefaultLength, parseState.Index);
        foreach (var member in _members)
        {
            var result = member.Initializer.Initialize(new StructureEvaluationContext(instance), span);
            if (result.Discard) break;
        }
        return instance;
    }
}
